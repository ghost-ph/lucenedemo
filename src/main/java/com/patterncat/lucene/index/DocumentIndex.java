package com.patterncat.lucene.index;

import java.io.*;
import java.util.*;

/**
 * This represents an index of documents. The index contains such information as
 * max_tf, doclen, and filename. Note that these are all of the members of
 * DocumentData.
 */
public class DocumentIndex implements Index {
    /**
     * The internal representation of the index. This maps a document ID to a
     * DocumentData Object, in which all data about the Document is stored.
     */
    private HashMap<Integer,DocumentData> index;
    ...
    public DocumentIndex() {
        index = new HashMap<Integer, DocumentData>();
    }

    /**
     * Build the document statistics based on statistics collected on a database
     */
    ...
    public synchronized void register(int docid, DocumentData data) {
    ...
    public void flush(String filename) throws IOException {
        ...
        Iterator<Integer> i = index.keySet().iterator();

        // these two are used in compression
        HashMap<String, Integer> fileIDHash = new HashMap<String, Integer>();
        int fileID = 0;
        ...
        while (i.hasNext()) {
            ...
            out.println(docid + " " + data.max_tf + " " + data.doclen + " "
                    + fileIDHash.get(data.filename));
        }
    ...
    /**
     * Load the index from the file based on the same format used in
     * store(String).
     */
    public void read(String filename) throws IOException {
        ...
        index = new HashMap<Integer, DocumentData>((int) (size / .75) + 1);
        ...
        while ((line = r.readLine()) != null) {
            ...
            docid = Integer.parseInt(tokens.nextToken());
    ...
    public Iterator<Integer> documents() {
