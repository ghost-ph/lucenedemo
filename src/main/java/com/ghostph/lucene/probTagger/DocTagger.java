package com.ghostph.lucene.probTagger;
...
public class DocTagger {
     ...
     * @return 估计的标注类型序列
     */
    public static PartOfSpeech[] hmm(ArrayList<DocTokenInf> ret) {
        DocTypes startType = new DocTypes();
        startType.put(new DocTypes.DocTypeInf(PartOfSpeech.start,1,0));
        ret.add(0, new DocTokenInf(-1,0,"Start",startType));

        DocTypes endType = new DocTypes();
        endType.put(new DocTypes.DocTypeInf(PartOfSpeech.end,100,100));
        ...
        int[][] prob = new int[stageLength][PartOfSpeech.values().length];//累积概率
        for(int i = 0;i<stageLength;++i)
        {
            //prob[i] = new int[PartOfSpeech.values().length];
            for(int j=0;j<PartOfSpeech.values().length;++j)
                prob[i][j] = Integer.MIN_VALUE;
        }

        PartOfSpeech[][] bestPre = new PartOfSpeech[stageLength][];//最佳前驱，也就是前一个标注是什么
        for(int i = 0;i<ret.size();++i)
        {
        ...
        PartOfSpeech endTag = PartOfSpeech.end;
        ...
        for (int i = 0; i < tokens.size(); ++i) {
            ...
            while (true) {
                ...
                if (i >= tokens.size()) {
                    ...
                    item.put(new DocTypes.DocTypeInf(PartOfSpeech.unknow, 10, 0));
    ...
    public static ArrayList<DocToken> tag(String addressStr) // 分词
    {
        ...
        ArrayList<DocToken> tokens = maxProb(g);
        //增加开始和结束节点
        DocToken startToken = new DocToken(-1,0,"Start",PartOfSpeech.start);
        tokens.add(0, startToken);
        DocToken endToken = new DocToken(g.verticesNum-1,g.verticesNum,"End",PartOfSpeech.end);
        tokens.add(endToken);
    ...
    public static void main(String[] args) {
        String addressStr = //"山西省太原市府东街32号";
                //"太原市半坡东街49号";
                //"候家巷1号";
                //"上海市卢湾区思南路113金码大厦旁";
                //"上海市中山北一路1250号后";
                //"红路175弄44号602";
                //"广东省东莞市莞温路３８３温塘中路";
                //"中国北京市朝阳区工体北路甲2号盈科中心商场一层109单元";
                //"山东省李强市朝山街112号";
                //"四川省德阳中江县凯江镇东河路下段1号";
                //"广东省广州市从化市神岗镇广从北路28号";
                //"广东省广州市从化市神岗镇";
                //"北京市朝阳区管庄东卫城A座1402";
                //"北京市丰台区南三环东路29号地铁5号线刘家窑站B出口旁";
                //"河北省邯郸市涉县索堡镇";
                //"上海市虹口区四川北路2146号";
                //"上海市虹口区吉祥路69号(近欧阳路)";
                //"北京东城区和平里七区16楼217室";
                //"北京市大兴工业开发区泰中花园3-15-204";
                //"中国 北京 北京市 丰台区大红门南路新华北里1号";
                //"市";
                //"云南省石林彝族自治县";
                "北京市朝阳";
