package com.ghostph.lucene.bigramSeg;
...
public class Segmenter {
    ...
    public ArrayDeque<CnToken> split(String sentence) {
        ...
        for (CnToken currentWord : segGraph) { // 从前往后遍历切分词图中的每个词
            // 得到当前词的前驱词集合
            CnTokenLinkedList prevWordList = segGraph
                    .prevWordList(currentWord.start);
            ...
            currentWord.bestPrev = minToken; // 设置当前词的最佳前驱词
    ...
    public ArrayDeque<CnToken> bestPath() {  //根据最佳前驱节点找切分路径
    ...
    // 前后两个词的转移概率
    private double transProb(CnToken prevWord, CnToken currentWord) {
        // TODO prob
